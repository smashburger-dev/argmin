...
| `katex`, `jsxgraph`, `pyodide`, `cpython`, `numpy`, `mathlive` | Software-Runtimes | open | versionsgenaue SPDX-Ausdrücke in `vendor/licenses/THIRD_PARTY_NOTICES.json` | jeweilige Upstream-Autor:innen | Public-Artefakte werden nur mit referenziertem Lizenztext, Quell-URL und geprüftem Hash gebaut (S1B: Numbas-Retirement; S1A: ts-fsrs-Retirement; danach: SymPy/mpmath-Retirement zugunsten des JS-Probe-Graders) | `tools/validate_content.mjs` erzwingt Notice-Abdeckung und Hashes; Details in `docs/dependency-matrix.md` |
