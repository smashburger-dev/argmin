...
Dritte Welle nach dem Rest-Audit: **Dreizehn weitere Fälle sind prozedural**, und ein zentraler Feedback-Defekt ist behoben. Für Lernende: Aufgabenhinweise und Fehlertexte nennen jetzt nur noch Zahlen und Optionen, die auch in der gezogenen Aufgabe stehen — bisher konnten authored Rückmeldungen aus dem Stamm-Fall auf eine andere Instanz feuern und dort falsche Werte oder falsche Optionen beschreiben. Bei Auswahlfragen mit rotierten Optionen werden die Rückmeldungen über den Optionstext an die richtige Position gebunden; Zahl-Regeln feuern nur noch, wenn die gezogenen Werte zum Text passen. Außerdem sind vier bisher unerreichbare git-Fälle (push, merge, Konflikt aufgelöst, Datei zurückholen) jetzt richtig im Kurs verankert.

Für Beitragende: `rebindAuthoredFeedback` in `family_registry.mjs` rebint authored `choice ===`-Regeln auf die semantische Option (Text → rotierte ID) und gated `value ===`-Regeln auf anker-identische Draws — falsche Erklärtexte können so nicht mehr auf Fremd-Draws feuern, die curated Regeln der Base-Fälle bleiben vollständig. Neue Prozeduralfälle: `grad-mse-numpy-reference` und `detect-goal-shift` als Pyodide-Fixture-Shards mit `__ref_`-Orakelkopie (goalshift zieht alle fünf Edge-Arme pro Instanz), `separate-error-kinds` als Sechs-Arm-Fixture-Shard (108 distinkte Fixtures), `journal-entry-order` als echte Parsons-Permutation (`contract: null`, `authorityMode: 'seeded'`, ~4.400 Ordnungen, authored Ordnungen erreichbar), die vier verbliebenen `matmul-entry`-/`dot-product`-w05-Fälle im `genMatmulEntry`/`genDot`-Raum (authored Anker inkl. `B[0][0]=0` erreichbar), `independent-sets-r2`/`r3` mit gezogenen Vektormengen, vier disjunkten Trap-Klassen und `rank()`-verifiziertem Solver, sowie `two-functions-one-print` als letzter statischer Fall von `trace-call-composition`. Die vier authored git-Fallkörper stehen jetzt in `caseTypes`. Zahlen- und positionsgebundene authored Hints/typicalErrors in acht Fallkörpern (Trace-Loops, Bibliotheks-Ausgaben, Toposort, 2×2-System) sind generisch umformuliert; `parameters.domain` dokumentiert die tatsächlichen Draw-Räume und Profil-Prädikate. `hintForMultipleChoice` nennt eine Eliminationsoption nur noch, wenn sie via `expectedAnswer.correctIds` belegbar falsch ist.
