...
function drawBaselineCase(r) {
  ...
  const ergebnis = { werte, digest: 'abc123' };
  ...
  // which verweigert arm: 0 protocol-metrik, 1 karten, 2 fehlerliste
  const brokenArm = randInt(r, 0, 2);
...
function baselineChecks(entry, index) {
  ...
  const args = [
    `[${py(entry.ergebnis)}, ${py(protoBroken)}, ${py(entry.karten)}, ${py(entry.fehlerliste)}]`,
    `[${py(entry.ergebnis)}, ${py(entry.protocol)}, ${py(kartenBroken)}, ${py(entry.fehlerliste)}]`,
    `[${py(entry.ergebnis)}, ${py(entry.protocol)}, ${py(entry.karten)}, ${py(fehlerBroken)}]`,
  ];
  return [
