...
predictCapsuleSuite('trace-library-api-output', mod, [
...
test('expected output: solver recomputes the prediction deterministically', () => {
  for (const caseId of CASE_IDS) {
    ...
    for (let seed = 0; seed < 200; seed += 1) {
      const generated = mod.genLibraryApiCase(seed, caseId, def);
  ...
  for (let seed = 0; seed < 50; seed += 1) {
    const p = mod.genLibraryApiCase(seed, 'pandas-dedup-isna-lines', mod.LIBRARY_API_CASES['pandas-dedup-isna-lines']).parameters;
    assert.equal(mod.pandasDedupIsnaOutput(p.ids, p.ziels), refPandas(p.ids, p.ziels));
    const n = mod.genLibraryApiCase(seed, 'numpy-median-histogram-corrcoef', mod.LIBRARY_API_CASES['numpy-median-histogram-corrcoef']).parameters;
    assert.equal(mod.numpySummaryOutput(n.x, n.bins, n.y), refNumpy(n.x, n.y, n.bins));
    const s = mod.genLibraryApiCase(seed, 'sklearn-split-no-shuffle', mod.LIBRARY_API_CASES['sklearn-split-no-shuffle']).parameters;
...
test('snippet carries the drawn literals', () => {
  const pandas = mod.genLibraryApiCase(0, 'pandas-dedup-isna-lines', mod.LIBRARY_API_CASES['pandas-dedup-isna-lines']);
  assert.ok(pandas.parameters.snippet.includes(`"id": [${pandas.parameters.ids.join(', ')}]`), 'pandas snippet carries ids');
  const numpy = mod.genLibraryApiCase(0, 'numpy-median-histogram-corrcoef', mod.LIBRARY_API_CASES['numpy-median-histogram-corrcoef']);
  ...
  const sklearn = mod.genLibraryApiCase(0, 'sklearn-split-no-shuffle', mod.LIBRARY_API_CASES['sklearn-split-no-shuffle']);
...
test('seeded draws stay inside the declared domains', () => {
  for (let seed = 0; seed < 200; seed += 1) {
    const pandas = mod.genLibraryApiCase(seed, 'pandas-dedup-isna-lines', mod.LIBRARY_API_CASES['pandas-dedup-isna-lines']).parameters;
    ...
    const numpy = mod.genLibraryApiCase(seed, 'numpy-median-histogram-corrcoef', mod.LIBRARY_API_CASES['numpy-median-histogram-corrcoef']).parameters;
    ...
    const sklearn = mod.genLibraryApiCase(seed, 'sklearn-split-no-shuffle', mod.LIBRARY_API_CASES['sklearn-split-no-shuffle']).parameters;
...
test('family extras: contract fields and broken-parameter error path', () => {
  ...
  const good = mod.genLibraryApiCase(0, 'sklearn-split-no-shuffle', mod.LIBRARY_API_CASES['sklearn-split-no-shuffle']).parameters;
