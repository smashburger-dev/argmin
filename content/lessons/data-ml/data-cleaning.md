...
Bearbeite [In einer Kundentabelle fehlt der Zielwert „Umsatz“ genau bei den Kunden mit …](#/family/classify-missingness/target-dependent-missingness/0/intro) (Konzeptfrage Verzerrung), danach die Abrufinstanzen dieser Lektion. Dann implementiere `profile_table` in [Implementiere profile_table(rows). Eingabe: rows ist eine Liste von …](#/family/validate-data-quality-contract/profile-table-schema-counts/0/core) und den Datenqualitätsvertrag als Final Boss in [Final Boss Datenqualitätsvertrag: Implementiere validate_rows(rows, contract). …](#/family/validate-data-quality-contract/validate-rows-contract-errors/0/stretch).
