# Terme strukturiert umformen

Eine Termumformung verändert die Darstellung, nicht den Wert. Du brauchst dafür zwei Fragen: Welche Operation verbindet die Teile? Welche Terme sind gleichartig?

## Klammer Schritt für Schritt auflösen

Vereinfache `3(x + 2) - 2(x - 1)`.

1. Verteile `3` auf beide Teile der ersten Klammer: `3x + 6`.
2. Verteile `-2` auf beide Teile der zweiten Klammer: `-2x + 2`.
3. Schreibe den ganzen Term: `3x + 6 - 2x + 2`.
4. Fasse die `x`-Terme zusammen: `3x - 2x = x`.
5. Fasse die Zahlen zusammen: `6 + 2 = 8`.
6. Ergebnis: `x + 8`.

Das Vorzeichen vor der zweiten Klammer gehört zum Faktor. Aus `-2(x - 1)` wird deshalb `-2x + 2`.

## Gleichartige Terme

`4x`, `-x` und `2x` sind gleichartig, weil sie denselben Variablenteil `x` besitzen. `4x` und `4x²` sind nicht gleichartig. Du darfst nur die Koeffizienten gleichartiger Terme addieren:

`4x - x + 2x = (4 - 1 + 2)x = 5x`.

## Kontrolle durch Einsetzen

Setze in Ausgangsterm und Ergebnis zum Beispiel `x = 2` ein:

- Ausgang: `3(2 + 2) - 2(2 - 1) = 12 - 2 = 10`.
- Ergebnis: `2 + 8 = 10`.

Ein einzelner Prüfwert beweist keine allgemeine Gleichheit. Er findet aber viele Vorzeichen- und Verteilungsfehler schnell.

## Kurzer Abruf

Vereinfache `5(2x - 1) - 3(x + 4)`. Markiere zuerst die beiden Verteilungen. Fasse danach die `x`-Terme und die Zahlen getrennt zusammen. Prüfe deinen Term mit `x = 1`.
