...
In einer Einstiegsaufgabe zählst du erlaubte Aktionen unter einer Richtlinie ab. [Baue den Prototyp-Kern: build_prototype(config) liefert {"answer": f, …](#/family/construct-stub-prototype-contract/stub-prototype-contract/0/core) baut `build_prototype` mit `answer` und `metrics`; [Vollständiger abgesicherter Prototyp: build_secure_prototype(config) erweitert …](#/family/construct-secure-prototype-contract/secure-prototype-contract/0/stretch) ergänzt `audit` und `request_action` zur vollen Kontrolle; der Boss [Final Boss Ablation: Implementiere ablation(config). config wie in w30-e5 plus …](#/family/reproduce-pipeline-status-report/pipeline-status-report/0/challenge) misst die Ablation mit/ohne Kontrolle gegen Referenzwerte. Das Runner-Projekt **rag-secure-prototype** führt dasselbe lokal mit pytest aus.
