...
In einer Einstiegsaufgabe zählst du Filter-Ergebnisse aus einem gelabelten Korpus ab. [Defensive Injektionserkennung: Implementiere contains_injection(text, rules) — …](#/family/aggregate-confusion-metric/contains-injection-rules/0/core) implementiert `contains_injection` samt numerischer Precision/Recall-Bewertung; [Least Privilege als Code: Implementiere check_permissions(tool_calls, policy). …](#/family/classify-rule-cascade-priority/permission-policy-check/0/stretch) prüft Toolberechtigungen gegen eine Least-Privilege-Policy; der Boss [Final Boss Detektor-Vergleich: Implementiere detector_table(corpus, rule_sets) …](#/family/aggregate-detector-eval-compare/detector-table-best-f1/0/challenge) vergibt eine Metrik-Tabelle über drei Regelwerke gegen Referenzwerte.
