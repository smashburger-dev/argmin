...
Berechne in einer Einstiegsaufgabe Recall-Werte aus generierten Rankings. In [Implementiere normalize(text) und chunk(text, size, overlap) exakt nach dem …](#/family/construct-normalize-chunk-contract/normalize-chunk-contract/0/core) implementierst du `chunk` und die gepinnte Normalisierung exakt; [Implementiere die deterministische Retrieval-Baseline mit NumPy. …](#/family/aggregate-retrieval-ranking-metric/retrieval-ranking-recall/0/stretch) verlangt TF-IDF-Kosinus-Suche und `recall_at_k` auf einer festen Dokumentmenge; der Boss [Final Boss Retrieval-System: Baue einen bewertbaren Mini-Suchindex über den …](#/family/aggregate-retrieval-ranking-metric/retrieval-evaluate-queries/0/challenge) baut daraus einen bewertbaren Mini-Retrieval-Index mit Recall@k und MRR gegen Referenzwerte.
