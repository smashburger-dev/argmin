...
In einer Einstiegsaufgabe aggregierst du ein Fehler-Ledger in bereinigte und naive Genauigkeit. [Code lesen und vorhersagen: Was gibt dieses Programm aus? Sage alle …](#/family/formula-ratio-percent-metric/ledger-rates-output-trace/0/core) sagst du die Ausgaben einer Ledger-Auswertung vorher; [Implementiere das gehashte Golden Set. build_golden(items) erhält eine Liste …](#/family/reproduce-canonical-hash-verify/canonical-hash-verify/0/core) baust du das gehashte Golden Set; [Implementiere die Baseline mit Reproduzierbarkeits-Assert. run_baseline(config) …](#/family/reproduce-run-digest-assert/run-digest-assert/0/stretch) implementierst du run_baseline mit Doppel-Lauf-Assert; der Boss [Final Boss Baseline-Report: Implementiere baseline_report(ergebnis, protocol, …](#/family/validate-report-guard-compose/baseline-report/0/challenge) verweigert Messwerte ohne Protokoll-Metrik, Karten und Fehlerliste.
