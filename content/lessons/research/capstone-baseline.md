...
In einer Einstiegsaufgabe aggregierst du ein Fehler-Ledger in bereinigte und naive Genauigkeit. In der [Kernaufgabe](#/family/formula-ratio-percent-metric/ledger-rates-output-trace/0/core) sagst du die Ausgaben einer Ledger-Auswertung vorher; in der [Kernaufgabe](#/family/reproduce-canonical-hash-verify/canonical-hash-verify/0/core) baust du das gehashte Golden Set; in der [Vertiefungsaufgabe](#/family/reproduce-run-digest-assert/run-digest-assert/0/stretch) implementierst du run_baseline mit Doppel-Lauf-Assert; die [Herausforderung](#/family/validate-report-guard-compose/baseline-report/0/challenge) als Boss verweigert Messwerte ohne Protokoll-Metrik, Karten und Fehlerliste.
