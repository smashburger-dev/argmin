...
Prüfe in einer Einstiegsaufgabe Shape- und Maskenzählungen. Rechne in [Stabiles Softmax lesen: Was gibt dieses Programm aus? Sage beide print-Zeilen …](#/family/trace-assignment-state/stable-softmax-rows-trace/0/core) ein stabiles Softmax per Hand nach. In [Implementiere attention(Q, K, V, mask=None) — Scaled Dot-Product Attention als …](#/family/optimize-softmax-attention-mask/scaled-dot-product-attention/0/core) implementierst du `attention(Q, K, V, mask)` mit Maskenfall; [Masken-Bausteine: Implementiere drei Funktionen. causal_mask(n) liefert die …](#/family/construct-attention-mask/construct-causal-padding-mask/0/stretch) verlangt Masken-Bausteine mit Fehlerbehandlung; [Final Boss Multi-Head: Implementiere multi_head_attention(X, Wq, Wk, Wv, Wo, …](#/family/optimize-multi-head-attention/multi-head-attention/0/challenge) ist der Multi-Head-Endgegner.
